...
async function run() {
  ...
  try {
    ...
    /*
     *Chunk long list of annotations such that the API doesn't reject it
     */
    const MAX_ANNOTATIONS_API = 50
    if(output.annotations.length > MAX_ANNOTATIONS_API) {
      let annotationsArrays = output.annotations.reduce((all,one,i) => {
        const ch = Math.floor(i/MAX_ANNOTATIONS_API)
        all[ch] = [].concat((all[ch]||[]),one)
        return all
      }, [])

      for(arr of annotationsArrays) {
        output.annotations = arr;
